...
def command_loop():
    ...
    while True:
        ...
        if query.lower() == 'exit':
            ...
            display_function_name = f"display_{key.lower().replace(' ', '_')}"
            if globals().get(display_function_name):
                globals()[display_function_name]()
            else:
...
if __name__ == "__main__":
    process_new_files()  # Process new files at startup
    command_loop()
